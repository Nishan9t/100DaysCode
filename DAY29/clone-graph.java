...
class Solution {
    ...
    {
        visited[copy.val]=copy; //store the current node at it's valindex which tell us that this node is now visited
        ...
        for(Node n:node.neighbors)
        {
            ...
            {
               // so not if it not visited , create a new node
                Node newNode = new Node(n.val);
                //add this node as the neighbout of the prev copied node
